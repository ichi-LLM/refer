...
class ExcelHandler:
    ...
    def __init__(self, config=None):
        ...
        self.trigger_sheet = None  # Trigger_editシート追加
    ...
    def create_requirement_excel(self, items: List[Dict], output_file: str, show_progress: bool = True) -> None:
        ...
        # シート作成順序を変更：User → System → Trigger → Requirement
        
        # 1. User_Description_editシートを作成
        if show_progress:
            print("\n[2/5] User Description編集シート作成中...")
        ...
        if show_progress:
            print("\n[3/5] System Description編集シート作成中...")
        ...
        # 3. Trigger_editシートを作成（新規追加）
        if show_progress:
            print("\n[4/5] Trigger編集シート作成中...")
        logger.info("Trigger編集シート作成開始...")
        self.trigger_sheet = self.wb.create_sheet("Trigger_edit")
        self._create_trigger_sheet(items)
        
        # 4. Requirement_of_Driverシートを作成
        if show_progress:
            print("\n[5/5] 要件一覧シート作成中...")
    ...
    # ===== Trigger_edit シート関連の新しいメソッド =====
    
    def _create_trigger_sheet(self, items: List[Dict]) -> None:
        """
        Trigger_editシートを作成（コンパクトテーブル形式）
        
        Args:
            items: 要件アイテムリスト
        """
        ws = self.trigger_sheet
        ...
        # トリガー要件をフィルタリング（階層名や要件名で判定）
        logger.info("トリガー要件をフィルタリング中...")
        trigger_items = []
        for idx, item in enumerate(items):
            # トリガー要件の判定条件（カスタマイズ可能）
            if self._is_trigger_requirement(item):
                trigger_items.append((item, idx))
        
        logger.info(f"トリガー要件数: {len(trigger_items)}件")
        
        if not trigger_items:
            logger.info("トリガー要件が見つかりませんでした")
            # 説明テキストを表示
            cell = ws["A1"]
            cell.value = "トリガー要件が見つかりませんでした"
            cell.font = Font(name=self.DEFAULT_FONT_NAME, size=12)
            
            # 使用方法の説明を追加
            instructions = [
                "",
                "【トリガー要件の作成方法】",
                "1. Requirement_of_Driverシートで新規要件を追加",
                "2. 階層名に「トリガー」を含める、または",
                "3. 要件名に特定のキーワード（例：通信異常、故障、エラー）を含める",
                "",
                "トリガー要件が検出されると、このシートに自動的にテンプレートが作成されます。"
            ]
            ...
            # タイトル行
            cell = ws["A1"]
            cell.value = "=== 異常系トリガー定義 ==="
            cell.font = Font(name=self.DEFAULT_FONT_NAME, bold=True, size=14)
            ws.merge_cells("A1:F1")
            
            # 使用方法
            cell = ws["A3"]
            cell.value = "各トリガーの「設定値」列を編集してシナリオ生成に使用します"
            cell.font = Font(name=self.DEFAULT_FONT_NAME, italic=True, color="666666", size=10)
            ws.merge_cells("A3:F3")
            
            # トリガーテンプレート作成
            current_row = 10
            
            for idx, (item, original_idx) in enumerate(trigger_items, 1):
                # 進捗表示（50件ごと）
                if idx % 50 == 0 or idx == len(trigger_items):
                    logger.info(f"Triggerテンプレート作成進捗: {idx}/{len(trigger_items)}")
                
                jama_id = item.get('jama_id', '新規')
                name = item.get('name', 'トリガー要件')
                
                # ヘッダー行（【JAMA_ID】要件名）
                cell = ws[f"A{current_row}"]
                cell.value = f"【{jama_id}】{name}"
                cell.font = Font(name=self.DEFAULT_FONT_NAME, bold=True, size=11)
                cell.fill = header_fill
                ...
                cell.value = "一覧に戻る"
                cell.hyperlink = f"#Requirement_of_Driver!A{original_idx + 2}"
                cell.font = Font(name=self.DEFAULT_FONT_NAME, color="0000FF", underline="single", size=10)
                ...
                cell = ws[f"B{current_row}"]
                # Descriptionから信号名を抽出（あれば）
                signal_name = self._extract_signal_name_from_description(item.get('description', ''))
                cell.value = signal_name if signal_name else f"SIGNAL_{idx}"
                cell.font = Font(name=self.DEFAULT_FONT_NAME)
                ...
                cell = ws[f"B{current_row}"]
                # 信号名からアクションタイプを推定
                action_type = self._infer_action_type(signal_name if signal_name else name)
        ...
        logger.info("Trigger編集シート作成完了")
    
    def _is_trigger_requirement(self, item: Dict) -> bool:
        """
        トリガー要件かどうかを判定
        ...
        # 階層情報から判定（必要に応じて実装）
        # sequence = item.get('sequence', '')
        # if sequence.startswith('1.2.3'):  # 特定の階層
        #     return True
        
        return False
    
    def _extract_signal_name_from_description(self, description: str) -> Optional[str]:
        """
        Descriptionから信号名を抽出
        
        Args:
            description: Description文字列
    ...
    def _infer_action_type(self, signal_or_name: str) -> str:
        """
        信号名または要件名からアクションタイプを推定
        
        Args:
            signal_or_name: 信号名または要件名
            
        Returns:
            推定されたアクションタイプ
        """
        name_lower = signal_or_name.lower()
        
        # マッピングルール
        if 'can' in name_lower or '通信' in name_lower:
            return 'can_communication_error'
        elif 'brake' in name_lower or 'ブレーキ' in name_lower:
            return 'brake'
        elif 'steering' in name_lower or 'ステアリング' in name_lower:
            return 'steering'
        elif 'accelerator' in name_lower or 'アクセル' in name_lower:
            return 'accelerator'
        elif 'sensor' in name_lower or 'センサ' in name_lower:
            return 'sensor_fault'
        elif 'powertrain' in name_lower or 'pt' in name_lower or 'パワトレ' in name_lower:
            return 'powertrain_fault'
        elif 'environmental' in name_lower or '環境' in name_lower or '外気' in name_lower:
            return 'environmental_fault'
        else:
            return 'can_communication_error'  # デフォルト