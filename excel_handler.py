...
class ExcelHandler:
    ...
    def read_requirement_excel(self, input_file: str) -> List[Dict]:
        ...
        try:
            ...
            for row_idx, row_num in enumerate(range(2, requirement_sheet.max_row + 1), 1):
                ...
                if operation == "新規":
                    hierarchy_info = self._calculate_hierarchy_info(requirement_sheet, row_num, all_items)
                    if hierarchy_info:
                        item.update(hierarchy_info)
                    else:
                        logger.warning(f"行{row_num}: 階層情報が不正です。スキップします。")
                        continue
    ...
    def _calculate_hierarchy_info(self, ws, row_num: int, all_items: List[Dict]) -> Optional[Dict]:
        """
        新規要件の階層情報から親要件とsequenceを計算
        ...
        if len(hierarchy) == 1:
            ...
            return {
                "name": name,
                "parent_id": None,
                "calculated_sequence": self._get_next_sequence("", all_items)
            }
        else:
            # 親階層の名前から親要件を検索
            parent_hierarchy = hierarchy[:-1]
            parent_item = self._find_parent_by_hierarchy(parent_hierarchy, all_items, ws)
            ...
            if parent_item:
                ...
                return {
                    ...
                    "calculated_sequence": self._get_next_sequence(parent_sequence, all_items)
                }
            else:
                logger.warning(f"親要件が見つかりません: {' > '.join(parent_hierarchy)}")
                return None
                
    def _find_parent_by_hierarchy(self, parent_hierarchy: List[str], all_items: List[Dict], ws) -> Optional[Dict]:
        """
        階層名のリストから親要件を検索
        ...
        for item in all_items:
            ...
            for col in range(4, 15):  # D=4, N=14
                value = ws.cell(row=row_num, column=col).value
                if value:
                    item_hierarchy.append(str(value).strip())
                else:
                    break
                    
            # 階層が一致するかチェック
            if item_hierarchy == parent_hierarchy:
                return item
    ...
    def validate_new_items(self, new_items: List[Dict], all_items: List[Dict]) -> List[str]:
        """
        新規作成アイテムのバリデーション
        
        Args:
            new_items: 新規作成アイテムリスト
            all_items: 全アイテムリスト（既存＋新規）
        ...
        for item in new_items:
            ...
            if item.get("parent_id"):
                ...
                for other in all_items:
                    if other.get("jama_id") == item["parent_id"]:
                        parent_found = True
                        break
                        
                if not parent_found:
                    errors.append(
                        f"要件「{item.get('name', 'Unknown')}」の親要件（ID: {item['parent_id']}）が見つかりません"
                    )
