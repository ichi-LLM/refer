...
class JAMAClient:
    ...
    def _process_item(self, item: Dict) -> Dict:
        ...
        # サンプルモードまたはデバッグモードで詳細出力
        if self.sample_mode or self.debug_mode:
            # 最初の3件とSYSPアイテムは必ず出力
        ...
        # 標準フィールドの取得（item直下とfields内の両方を確認）
        # status - システムフィールド
        result["status"] = item.get("status") or fields.get("status") or fields.get("Status") or ""
        
        # assigned/assignedUser - ユーザーフィールド
        result["assignee"] = (
            item.get("assigned") or 
            item.get("assignedUser") or 
            fields.get("assigned") or 
            fields.get("assignedUser") or 
        ...
        result["tags"] = item.get("tags") or fields.get("tags") or fields.get("Tags") or ""
        ...
        result["preconditions"] = (
            fields.get("preconditions") or 
            fields.get("Preconditions") or 
            fields.get("precondition") or 
            fields.get("Precondition") or 
            ""
        )
        ...
        result["target_system"] = (
            fields.get("target_system") or 
            fields.get("Target_system") or 
            fields.get("targetSystem") or 
            fields.get("TargetSystem") or 
            fields.get("target") or 
            fields.get("Target") or 
            ""
        )
        
        # SYSPアイテムの場合、追加のデバッグ情報
        if "SYSP" in name and (self.debug_mode or self.sample_mode):
