...
class JAMAClient:
    ...
    def create_item(self, item_data: Dict) -> int:
        ...
        logger.info(f"新規作成: 階層レベル={hierarchy_level}, item_type_id={item_type_id}")
        
        # リクエストボディ作成
        request_data = {
            "project": self.project_id,
            "itemType": item_type_id,
            "childItemType": item_type_id,  # 同じ値を設定（将来的に変更可能）
            "fields": self._prepare_fields(item_data)
        }
        
        # locationの設定（親要件またはプロジェクト）
        ...
        # sequenceが計算されている場合は設定
        if item_data.get("calculated_sequence"):
            request_data["location"]["sequence"] = item_data["calculated_sequence"]
            logger.info(f"新規作成: {item_data.get('name', 'Unknown')}, sequence: {item_data['calculated_sequence']}")
        
        response = self._make_request("POST", "/rest/v1/items", json_data=request_data)
        created_id = response.get("meta", {}).get("id")
        
        if created_id:
            logger.info(f"アイテム作成成功: ID={created_id}")
            return created_id
        else:
            logger.error(f"アイテム作成レスポンスにIDが含まれていません: {response}")
            raise Exception("アイテム作成に失敗しました")
            
    def _calculate_hierarchy_level(self, item_data: Dict) -> int:
        """
        アイテムデータから階層レベルを計算
        ...
        Returns:
            階層レベル（1-11）
        """
        # calculated_sequenceから階層レベルを判定
        sequence = item_data.get("calculated_sequence", "")
        ...
        if sequence:
            ...
            logger.info(f"Sequence '{sequence}' から階層レベル {hierarchy_level} を判定")
